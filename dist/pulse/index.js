...
const _ID = Symbol((0, _v.default)());

const _attempt = Symbol((0, _v.default)());

const _attempts = Symbol((0, _v.default)());

const _autostart = Symbol((0, _v.default)());

const _callback = Symbol((0, _v.default)());

const _count = Symbol((0, _v.default)());

const _complete = Symbol((0, _v.default)());

const _delay = Symbol((0, _v.default)());

const _error = Symbol((0, _v.default)());

const _params = Symbol((0, _v.default)());

const _pending = Symbol((0, _v.default)());

const _registry = Symbol((0, _v.default)());

const _repeat = Symbol((0, _v.default)());

const _timer = Symbol((0, _v.default)());

const _status = Symbol((0, _v.default)());

const debug = (...args) => {
  if (_enums.default.DEBUG !== true) {
    return;
  }
...
class Pulse {
  constructor() {
    this.Task = PulseTask;
    this.ENUMS = _enums.default;
    this[_registry] = {};
  }

  get(ID) {
    return _objectPath.default.get(this[_registry], ID);
  }

  register(ID, callback, options = {}, ...params) {
    if (!ID) {
      throw new Error(_enums.default.ERROR.ID);
      return;
    }

    if (!callback) {
      throw new Error(_enums.default.ERROR.CALLBACK);
      return;
    }
    ...
    if (ID) {
      ...
      if (task) {
        ...
        const ival = setInterval(() => {
          if (task.status === _enums.default.STATUS.RUNNING) {
            return;
          }
  ...
  startAll() {
    Object.values(this[_registry]).filter(task => Boolean(task.status !== _enums.default.STATUS.STARTED)).forEach(task => task.start());
    return this;
  }
...
class PulseTask {
  constructor(options = {}, params) {
    this[_callback] = _objectPath.default.get(options, 'callback');
    this[_status] = _enums.default.STATUS.READY;
    this[_ID] = _objectPath.default.get(options, 'ID', (0, _v.default)());
    this[_attempt] = 0;
    this[_attempts] = _objectPath.default.get(options, 'attempts', _enums.default.DEFAULT.ATTEMPTS);
    this[_autostart] = _objectPath.default.get(options, 'autostart', _enums.default.DEFAULT.AUTOSTART);
    this[_complete] = false;
    this[_count] = 0;
    this[_delay] = _objectPath.default.get(options, 'delay', _enums.default.DEFAULT.DELAY);
    this[_error] = null;
    this[_params] = params;
    this[_repeat] = _objectPath.default.get(options, 'repeat', _enums.default.DEFAULT.REPEAT);
    this[_repeat] = isNaN(this[_repeat]) ? _enums.default.DEFAULT.REPEAT : this[_repeat];
    this[_timer] = null;
  ...
  get progress() {
    if (this.repeat === _enums.default.DEFAULT.REPEAT) {
      return 0;
    }
  ...
  onSuccess(result) {
    ...
    if (this.status === _enums.default.STATUS.STOPPED || this[_pending] === _enums.default.STATUS.STOPPED) {
      return this.stop();
    }
  ...
  onError(err) {
    debug('onError()', this);
    this[_error] = err;
    this[_status] = _enums.default.STATUS.ERROR;
  ...
  async now() {
    if (this.status === _enums.default.STATUS.RUNNING) {
      return;
    }

    debug('now()', this);
    this.stop();
    this[_status] = _enums.default.STATUS.RUNNING;
  ...
  start() {
    ...
    if (this[_timer] === null) {
      this[_timer] = setTimeout(() => this.now(this, this[_params]), this[_delay]);
    }
  ...
  stop() {
    ...
    if (this.status === _enums.default.STATUS.RUNNING) {
      this[_pending] = _enums.default.STATUS.STOPPED;
      return this;
    }

    clearTimeout(this[_timer]);
    this[_complete] = this[_count] >= this[_repeat];
    this[_status] = _enums.default.STATUS.STOPPED;
    this[_pending] = null;
    this[_timer] = null;
    return this;
  }

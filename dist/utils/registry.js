...
class Registry {
  ...
  get listById() {
    ...
    const registered = Array.from(this.__registered).filter(item => !(item[this.__idField] in unregister) && !(item[this.__idField] in banned));
  ...
  ban(id) {
    const ids = _underscore.default.flatten([id]);

    ids.forEach(id => _objectPath.default.set(this.__banned, [id], id));
  ...
  cleanup(id) {
    const [remove] = _underscore.default.flatten([id]);

    if (this.isProtected(remove)) return this;
    this.__registered = this.__registered.filter(item => item[this.__idField] !== remove);
    this.notify({
      type: 'cleanup',
      id
    });
  ...
  protect(id) {
    ...
    ids.forEach(id => _objectPath.default.set(this.__protected, [id], id));
    this.notify({
      type: 'protect',
      id
    });
  ...
  register(id, data = {}) {
    ...
    if (!id) id = (0, _v.default)();

    if (this.isBanned(id)) {
      return new Error(`${this.__name} unable to register banned item ${id}`);
    }

    if (this.isProtected(id) && this.isRegistered(id)) {
      return new Error(`${this.__name} unable to replace protected item ${id}`);
    }
  ...
  unprotect(id) {
    ...
    ids.forEach(id => _objectPath.default.del(this.__protected, id));
    this.notify({
      type: 'unprotect',
      id
    });
  ...
  unregister(id) {
    ...
    const ids = _underscore.default.chain([id]).flatten().uniq().value();
    ...
    ids.forEach(id => {
      ...
      _objectPath.default.set(this.__unregister, [id], id);
    });
    this.notify({
      type: 'unregister',
      id
